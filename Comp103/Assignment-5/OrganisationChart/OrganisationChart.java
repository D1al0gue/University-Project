...
public class OrganisationChart {
    ...
    private Position findPosition(double x, double y, Position pos){
        ...
        for (Position member : pos.getTeam()) {
            Position found = findPosition(x, y, member);
            if (found != null) {
                ans = found;
            }
        }
    ...
    public void addNewPosition(Position newPos, Position target){
        /*# YOUR CODE HERE */
        if (newPos == null || target == null || target.isManager()) {
            return; // Invalid arguments
        }
        
        target.addToTeam(newPos);
        newPos.setOffset(0); // Set initial offset
        selectedPosition = null; // Deselect after adding
        this.redraw();
    }
    ...
    private void movePosition(Position pos, Position target) {
        ...
        if (pos.getManager() != null){
            pos.getManager().removeFromTeam(pos); // Remove from current manager's team
        }

        target.addToTeam(pos); // Add to the target's team
        selectedPosition = null; // Deselect after moving
        this.redraw();
    }
    ...
    public void removePosition(Position pos){
        /*# YOUR CODE HERE */
        if (pos == null || pos.isManager() || pos.getManager() == null) {
            return; // Invalid arguments or position is a manager or root
        }
        
        pos.getManager().removeFromTeam(pos);
        selectedPosition = null; // Deselect after removal
        this.redraw();
    }
    ...
    private boolean inSubtree(Position pos, Position subtree) {
        ...
        for (Position member : subtree.getTeam()) {
            if (inSubtree(pos, member)) {
                return true;
            }
        }
