...
public class OrganisationChart {
    ...
    public static void main(String[] arguments) {
        ...
        UI.println("""
         Completed position.java (as instructed)

         core

         completion

         challenge (except for animation)
    ...
    public void setupGUI() {
        ...
        UI.addButton("Save Chart", this::saveChart); // Added Save button
        UI.addButton("Load Chart", this::loadChart); // Added Load button
    ...
    private void saveChart() {
        try {
            String fileName = UIFileChooser.save("Save Chart");
            if (fileName != null) {
                PrintWriter writer = new PrintWriter(new File(fileName));
                saveChartHelper(organisation, writer);
                writer.close();
            }
        } catch (IOException e) {
            UI.println("Error saving chart to file: " + e.getMessage());
        }
    }

    // [CHALLENGE:]
    private void saveChartHelper(Position pos, PrintWriter writer) {
        writer.println(pos.toStringFull()); // Save position details
        for (Position teamMember : pos.getTeam()) {
            saveChartHelper(teamMember, writer); // Recursive call for team members
        }
    }
    ...
    private void loadChart() {
        try {
            String fileName = UIFileChooser.open("Load Chart");
            if (fileName != null) {
                Scanner scanner = new Scanner(new File(fileName));
                Position newOrganisation = loadChartHelper(scanner);
                if (newOrganisation != null) {
                    organisation = newOrganisation; // Update the chart
                    selectedPosition = null; // Reset selected position
                    this.redraw();
                }
            }
        } catch (IOException e) {
            UI.println("Error loading chart from file: " + e.getMessage());
        }
    }

    // [CHALLENGE:]
    private Position loadChartHelper(Scanner scanner) {
        if (scanner.hasNextLine()) {
            String[] parts = scanner.nextLine().split(" ");
            String role = parts[0];
            double offset = Double.parseDouble(parts[1]);
            int teamSize = Integer.parseInt(parts[2]);
            Position pos = new Position(role, offset);

            for (int i = 0; i < teamSize; i++) {
                Position teamMember = loadChartHelper(scanner);
                if (teamMember != null) {
                    pos.addToTeam(teamMember);
                }
            }
            return pos;
        }
    ...
    private Position findPosition(double x, double y, Position pos){
        ...
        for (Position teamMember : pos.getTeam()) {
            Position found = findPosition(x, y, teamMember);
            if (found != null) {
                return found;
            }
        }
    ...
    private void movePosition(Position pos, Position target) {
        ...
        /*# YOUR CODE HERE */
        if (pos != null && target != null && pos != target && !inSubtree(pos, target)) {
            Position manager = pos.getManager();
            if (manager != null) {
                manager.removeFromTeam(pos);
            }
            target.addToTeam(pos);
            pos.moveOffset(target.getXCoordinate()); // Use the new method
            this.redraw();
        }
    }
    ...
    private boolean inSubtree(Position pos, Position subtree) {
        ...
        for (Position teamMember : subtree.getTeam()) {
            if (inSubtree(pos, teamMember)) {
                return true;
            }
        }
        return false;
