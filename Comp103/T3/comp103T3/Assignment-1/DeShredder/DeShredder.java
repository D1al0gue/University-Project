...
public class DeShredder {
    ...
    public void completeStrip(){
        ...
        if (completedStrips.size() > 0 ){ //if the size is greater than 0, add the strip to the end.
            completedStrips.add(completedStrips.size(), workingStrip);
        } else {
            completedStrips.add(0, workingStrip);
        }

        workingStrip = new ArrayList<Shred>();//initialise new working strip
        display();
    }
        ...
        if (action.equals("released")){
            ...
            if (fromStrip != null && toStrip != null){
                if (fromStrip == allShreds && toStrip == workingStrip){
                    ...
                    workingStrip(toStrip, toPosition); //moving shreds around the workingstrip & from Workingstrip to allshreds
                } else if (completedStrips.contains(fromStrip)){
                    completeStrips(toStrip); //moving completedstrips
                }
            }
            display();
        }
    ...
    //moves shreds from allshreds into workingstrips
    ...
    public void completeStrips(List<Shred> toStrip){ 
