...
public class HospitalERCore{
    ...
    private int totalPriority1Treated; 
    private int totalPriority1WaitingTime;
    ...
    public void reset(boolean usePriorityQueue){
        ...
        totalWaitingTime = 0;
        totalPriority1Treated = 0; //reset the priority 1 statistics
        totalPriority1WaitingTime = 0; //reset the priority 1 statistics
    ...
    public void run(){
        ...
        while (running){         // each time step, check whether the simulation should pause.
            ...
            for (Patient patient : treatmentRoom){
                if (patient.currentTreatmentFinished()) {
                    ...
                    if (patient.getPriority() == 1){
                        totalPriority1Treated++;
                    }
                }
            }

            for (Patient patient : patientsToDischarge) {
                UI.println(time + ": Discharge: " + patient);
                treatmentRoom.remove(patient);
            }


            // Process one time tick for each patient in the treatment room and waiting room
            for (Patient patient : treatmentRoom) {
            ...
            while (!waitingRoom.isEmpty() && treatmentRoom.size() < MAX_PATIENTS) {
                ...
                UI.println(time + ": Move to treatment: " + nextPatient);
            }
    ...
    public void reportStatistics(){
        ...
        UI.println("Total number of patients treated: " + totalPatientsTreated);
        if (totalPatientsTreated > 0) {
            double averageWaitingTime = (double) totalWaitingTime / totalPatientsTreated;
            UI.printf("Average waiting time of patients: %.2f minutes\n", averageWaitingTime);
        } else {
            UI.println("No patients treated.");
        }

        UI.println("Total number of priority 1 patients treated: " + totalPriority1Treated);
        if (totalPriority1Treated > 0) {
            double averagePriority1WaitingTime = (double) totalPriority1WaitingTime / totalPriority1Treated;
            UI.printf("Average waiting time of priority 1 patients: %.2f minutes\n", averagePriority1WaitingTime);
        } else {
            UI.println("No priority 1 patients treated.");
        }
    }
