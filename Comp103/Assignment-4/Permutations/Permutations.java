...
public class Permutations {
    ...
    public void extendPermutation(Set<String> remainingItems, Stack<String> permutationSoFar, List<List<String>> allPermutations){
        /*# YOUR CODE HERE */
        // Base case: If there are no remaining items, add a copy of the permutationSoFar to allPermutations
        if (remainingItems.isEmpty()) {
        allPermutations.add(new ArrayList<>(permutationSoFar));
        counter++; // Increment the permutation counter
        reportCounter(); // Print the current permutation count
        return;
    }

    // Limit the number of permutations to 10,000
    if (allPermutations.size() >= 10000) {
        return;
    }

    // Recursive case: Iterate through remaining items and extend the permutation
    for (String item : new HashSet<>(remainingItems)) {
        // Remove the item from remaining items
        remainingItems.remove(item);

        // Push the item onto the permutation so far
        permutationSoFar.push(item);

        // Recursively extend the permutation
        extendPermutation(remainingItems, permutationSoFar, allPermutations);

        // Pop the item from the permutation so far
        permutationSoFar.pop();

        // Put the item back into remaining items for the next iteration
