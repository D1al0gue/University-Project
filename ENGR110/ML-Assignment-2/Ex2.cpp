...
/*SWAPPED 0'S AND 1'S HERE. WARMING THIS CODE WILL NOT DO ANYTHING (AT LEAST ANYTHING USEFUL)*/
...
void gradient_search(Neuron& neuron,const train_set_t& train_set){
    ...
    while ((current_tot_err>0.01)&&(n_step<150)){
		...
		for (uint i = 0 ; i < train_set.train_points.size(); i++){
		    ...
		    double y = neuron.y;
			double t = train_set.train_points[i].target;
			double delta = y * (1 - y) * (y - t);
